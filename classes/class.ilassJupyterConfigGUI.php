...
/**
 * Jupyter config gui
 * General settings for jupyter lab plugin
 *
...
class ilassJupyterConfigGUI extends ilPluginConfigGUI
{
	...
	protected function initConfigurationForm()
	{
		...
		$this->getPluginObject()->includeClass('class.ilJupyterUtil.php');
		
		$configured = $settings->getLanguages();
		foreach (ilJupyterUtil::getAvailableLanguages() as $lng_key)
		{
	 ...
	 * Read available mids
	 *
	 * @param ilJupyterSettings $settings
	 * @return string[string]
	 */
	protected function readAvailabeMids(ilJupyterSettings $settings, $a_only_self = false)
	{
		try
		{
