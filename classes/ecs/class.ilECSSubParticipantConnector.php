...
class ilECSSubParticipantConnector extends ilECSConnector
{
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Add new sub participant resource...');
	 	...
	 	try 
	 	{
			...
			{
				ilLoggerFactory::getLogger('jupyter')->error('Cannot create subparticipant, did not recieve HTTP 201.');
				ilLoggerFactory::getLogger('jupyter')->error('POST was: ' . json_encode($sub));
				ilLoggerFactory::getLogger('jupyter')->error('HTTP code was: ' . $info);
			...
			$ressource->setRessourceType(ilECSJupyterRessource::RES_SUBPARTICIPANT);
			$ressource->create();
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Delete subparticipant with id: '. $a_sub_id);
	 	$this->path_postfix = self::RESOURCE_PATH;
	...
	protected function parseResponse($return_str)
	{
		...
		ilLoggerFactory::getLogger('jupyter')->debug('Location:' . $location);
		ilLoggerFactory::getLogger('jupyter')->debug('id:' . $id);
		ilLoggerFactory::getLogger('jupyter')->debug('result-json' . print_r($ecs_result->getResult(), true));
