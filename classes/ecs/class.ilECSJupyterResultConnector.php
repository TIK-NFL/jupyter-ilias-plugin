 ...
 * 
 */
class ilECSJupyterResultConnector extends ilECSConnector
{
	...
	public function getResult($a_id)
	{
		...
		try {
			...
			{
				ilLoggerFactory::getLogger('jupyter')->warning('Cannot read jupyter result, did not receive HTTP 200');
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Add new result resource for subparticipant: '.$a_receiver_com);
	 	...
	 	try 
	 	{
			...
			{
				ilLoggerFactory::getLogger('jupyter')->error('Cannot create result, did not receive HTTP 201. ');
				ilLoggerFactory::getLogger('jupyter')->error(print_r($ret,true));
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Delete result with id: ' . $a_exc_id);
	 	$this->path_postfix = self::RESOURCE_PATH;
