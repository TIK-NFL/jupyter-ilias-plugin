...
class ilECSPointsConnector extends ilECSConnector
{
	...
	public function getPoints($a_id)
	{
		...
		ilLoggerFactory::getLogger('jupyter')->debug('getPoints() using uri:' . $this->getServer()->getServerURI().$this->path_postfix);
		...
		try {
			...
			{
				ilLoggerFactory::getLogger('jupyter')->warning('Cannot read jupyter points, did not receive HTTP 200');
