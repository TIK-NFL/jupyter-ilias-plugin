...
class ilECSExerciseConnector extends ilECSConnector
{
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Add new exercise ressource for subparticipant: '. print_r($a_receiver_com,true));
	 	...
	 	try 
	 	{
			...
			{
				ilLoggerFactory::getLogger('jupyter')->error('Cannot create exercise ressource, did not receive HTTP 201');
			...
			$ressource->setRessourceType(ilECSJupyterRessource::RES_EXERCISE);
			$ressource->create();
	...
	{
		ilLoggerFactory::getLogger('assjupyter')->debug('Delete exercise with id '. $a_exc_id);
	 	$this->path_postfix = self::RESOURCE_PATH;
