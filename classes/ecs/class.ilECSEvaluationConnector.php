...
class ilECSEvaluationConnector extends ilECSConnector
{
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Add new evaluation ressource for subparticipant: ' . $a_receiver_com);
	 	...
	 	try 
	 	{
			...
			ilLoggerFactory::getLogger('jupyter')->dump($exercise, ilLogLevel::DEBUG);
			...
			{
				ilLoggerFactory::getLogger('jupyter')->error('Cannot create evaluation ressource, did not receive HTTP 201');
			...
			$ressource->setRessourceType(ilECSJupyterRessource::RES_EVALUATION);
			$ressource->create();
	...
	{
		ilLoggerFactory::getLogger('jupyter')->debug('Delete evaluation with id: '. $a_exc_id);
	 	$this->path_postfix = self::RESOURCE_PATH;
